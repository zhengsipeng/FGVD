...
#get data from VOC or sample dataset
...
model_rpn = Model(img_input, rpn[:2])#rpn只返回x_class, x_regr
...
for epoch_num in range(num_epochs):
    ...
    while True:
        # data_gen_train是一个迭代器。返回的是 np.copy(x_img), [np.copy(y_rpn_cls), 
        #np.copy(y_rpn_regr)], img_data_aug（我们这里假设数据没有进行水平翻转等操作。
